...
export async function exportTransactionsCsv(req: Request, res: Response) {
  ...
  let list = await repo.transactions.list({ customerId, sourceType, sourceId, direction, from, to });
  list = [...list].sort((a, b) => new Date(a.occurredAt).getTime() - new Date(b.occurredAt).getTime());
...
export async function exportCustomersCsv(_req: Request, res: Response) {
  const repo = getRepo();
...
export async function exportChargesCsv(req: Request, res: Response) {
  const repo = getRepo();
  const { customerId } = req.query as Record<string, string | undefined>;
  const list = await repo.charges.list({ customerId });
  res.setHeader("Content-Type", "text/csv");
...
export async function exportStatementPdf(req: Request, res: Response) {
  const repo = getRepo();
  const cust = await repo.customers.get(String((req.params as Record<string,string>).customerId));
  if (!cust) return res.status(404).json({ error: "customer not found" });
  const from = req.query.from as string | undefined;
  const to = req.query.to as string | undefined;
  let txs = (await repo.transactions.list({ customerId: cust.id })).filter((t) => t.customerId === cust.id);
  if (from) txs = txs.filter((t) => new Date(t.occurredAt) >= new Date(from));
  if (to) txs = txs.filter((t) => new Date(t.occurredAt) <= new Date(to));
  txs = [...txs].sort((a, b) => new Date(a.occurredAt).getTime() - new Date(b.occurredAt).getTime());
  const charges = (await repo.charges.list({ customerId: cust.id })).sort((a, b) => a.periodMonth.localeCompare(b.periodMonth));
  ...
  let running = 0;
  for (const t of txs) {
    if (t.direction === "debit") running += t.amountPaise;
    else running -= t.amountPaise;
