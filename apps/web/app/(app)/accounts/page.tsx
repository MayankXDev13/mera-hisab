...
export default function AccountsPage() {
  ...
  const [form, setForm] = useState<{
    ...
    opening: string;
  }>({ name: "", type: "savings", opening: "" });
  const [editForm, setEditForm] = useState<{
    name: string;
    type: "savings" | "current";
    status: "active" | "deactivated";
  }>({ name: "", type: "savings", status: "active" });

  const submitCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    const paise = amountToPaiseOrNull(form.opening);
    if (form.opening.trim() && paise === null) {
    ...
    try {
      ...
      toast.add({
        ...
        description: formatRupees(openingBalancePaise),
        type: "success",
      });
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="sm:max-w-120">
            <DialogHeader>
              ...
              <DialogDescription>Add a bank or cash account.</DialogDescription>
            </DialogHeader>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setForm({
                      ...form,
                      type: (v ?? "savings") as "savings" | "current",
                    })
                  }
                >
                  <SelectTrigger>
          ...
          {isLoading ? (
            ...
            <div className="p-10 text-center">
              <div className="mx-auto size-12 rounded-2xl bg-muted grid place-items-center">
                <RiWallet3Line className="size-6 text-muted-foreground" />
              </div>
              <p className="text-sm font-medium mt-3">No accounts yet</p>
              <p className="text-xs text-muted-foreground mt-1">
                Add your first funding source to start lending.
              </p>
              <Button
                  ...
                  {data.map((a) => (
                    <TableRow key={a.id} className="group">
                      ...
                      <TableCell className="font-mono text-xs text-right text-muted-foreground">
                        {formatRupees(a.openingBalancePaise)}
                      </TableCell>
      ...
      >
        <DialogContent className="sm:max-w-120">
          <DialogHeader>
            ...
            <DialogDescription>Update name, type or status.</DialogDescription>
          </DialogHeader>
          {editingId && (
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setEditForm({
                      ...editForm,
                      status: (v ?? "active") as "active" | "deactivated",
                    })
                  }
                >
                  <SelectTrigger>
