          ...
          <DialogContent className="sm:max-w-130">
            <DialogHeader>
              ...
              <DialogDescription>
                {editing
                  ? "Update details."
                  : "Add a new khata. Username must be unique."}
              </DialogDescription>
            </DialogHeader>
        ...
        <CardHeader className="flex flex-row items-center justify-between gap-3 py-4 border-b bg-card">
          <div>
            <CardTitle className="text-[13px] flex items-center gap-2">
              <span className="size-7 rounded-lg bg-muted grid place-items-center">
                <RiGroupLine className="size-3.5" />
              </span>{" "}
              All customers
            </CardTitle>
            <p className="text-xs text-muted-foreground mt-1 hidden sm:block">
              Tap a row to view details and repayments.
            </p>
          </div>
          <div className="relative w-full max-w-65">
            ...
            <Input
              ...
              className="pl-8 h-8 rounded-full bg-background"
            />
          </div>
        </CardHeader>
          ...
          {isLoading ? (
            ...
            <div className="p-10 text-center">
              <div className="mx-auto size-12 rounded-2xl bg-muted grid place-items-center">
                <RiGroupLine className="size-6 text-muted-foreground" />
              </div>
              ...
              <p className="text-xs text-muted-foreground mt-1 max-w-[28ch] mx-auto">
                {search
                  ...
                  : "Create your first khata to start recording."}
              </p>
              {!search && (
                <Button
                    ...
                    >
                      ...
                      <TableCell className="text-right">
                        {outstandingMap.isLoading ? (
                          ...
                          <span
                            className={`font-mono text-sm font-medium ${(outstandingMap.data?.[c.id] ?? 0) > 0 ? "text-destructive" : (outstandingMap.data?.[c.id] ?? 0) < 0 ? "text-emerald-600 dark:text-emerald-400" : "text-muted-foreground"}`}
                          >
                      ...
                      <TableCell>
                        <Badge
                          ...
                          className="capitalize rounded-full text-[11px]"
                        >
                          {c.status}
                        </Badge>
                      </TableCell>
            ...
            <div className="space-y-5 mt-6">
              <div className="grid grid-cols-2 gap-3">
                <OutstandingBreakdown
                  ...
                  onRecord={() => setRepayOpen(true)}
                />
                <Card className="bg-muted/30">
                  <CardContent className="p-4">
                    <div className="text-[10px] tracking-[0.12em] uppercase text-muted-foreground font-medium">
                      Monthly rate
                    </div>
                    <div className="font-mono text-lg font-semibold mt-1">
                      {formatBps(selected.monthlyRateBps)}
                    </div>
              ...
              {selected.notes && (
                <div className="rounded-xl border bg-muted/20 p-4">
                  <div className="text-[11px] tracking-[0.08em] uppercase text-muted-foreground font-medium mb-1">
                    Notes
                  </div>
              ...
              <Dialog open={txnOpen} onOpenChange={setTxnOpen}>
                <DialogContent className="max-w-lg">
                  <DialogHeader>
                    <DialogTitle>New entry — {selected.name}</DialogTitle>
                    <DialogDescription>Record money given.</DialogDescription>
                  </DialogHeader>
                  <TransactionForm
              ...
              <Dialog open={repayOpen} onOpenChange={setRepayOpen}>
                <DialogContent className="max-w-md">
                  <DialogHeader>
                    <DialogTitle>
                      Record repayment — {selected.name}
                    </DialogTitle>
                    <DialogDescription>Money received back.</DialogDescription>
                  </DialogHeader>
                  <RepaymentContainer
                    customerId={selected.id}
                    customerName={selected.name}
                    onDone={() => setRepayOpen(false)}
                  />
                </DialogContent>
              </Dialog>
            </div>
...
function OutstandingBreakdown({
  ...
  if (q.isLoading) return <Skeleton className="h-35 w-full rounded-xl" />;
  if (q.error)
    return <div className="text-xs text-destructive">Failed to load</div>;
  const data = q.data;
  const positive = (data?.outstandingPaise ?? 0) > 0;
  return (
    <Card
      className={`${positive ? "border-destructive/15 bg-destructive/5 dark:bg-destructive/10" : "bg-muted/30"} overflow-hidden`}
    >
      <CardContent className="p-4">
        <div className="text-[10px] tracking-[0.12em] uppercase text-muted-foreground font-medium">
          Outstanding
        ...
        <div
          className={`font-mono text-lg font-semibold mt-1 ${positive ? "text-destructive" : ""}`}
        >
          {formatRupees(data?.outstandingPaise ?? 0)}
        </div>
        <div className="mt-3 space-y-1.5 max-h-24 overflow-auto pr-1">
          {(data?.sources ?? []).map((src) => (
            ...
            >
              ...
              <span className="font-mono font-medium shrink-0">
                {formatRupees(src.outstandingPaise)}
              </span>
            </div>
          ))}
          {!data?.sources.length && (
        ...
        <Button
          ...
          onClick={onRecord}
          disabled={(data?.outstandingPaise ?? 0) <= 0}
        >
          Record repayment
        </Button>
      </CardContent>
