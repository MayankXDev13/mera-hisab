...
export default function TransactionsPage() {
  ...
  const cardMap = new Map(
    cards.data?.map((c) => [c.id, `${c.issuer} ••${c.last4}`]) ?? [],
  );

  const totalPages = tx.data
    ? Math.max(1, Math.ceil(tx.data.total / tx.data.limit))
    : 1;
        ...
        <CardContent className="grid gap-3 md:grid-cols-3 lg:grid-cols-6">
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(v) =>
                setFilters({
                  ...filters,
                  customerId: v === "all" ? undefined : (v as string),
                  page: 1,
                })
              }
            >
            ...
            <Select
              ...
              onValueChange={(v) =>
                setFilters({
                  ...filters,
                  direction:
                    v === "all" ? undefined : (v as "debit" | "credit"),
                  page: 1,
                })
              }
            >
            ...
            <Select
              ...
              onValueChange={(v) =>
                setFilters({
                  ...filters,
                  sourceType:
                    v === "all" ? undefined : (v as "account" | "credit_card"),
                  sourceId: undefined,
                  page: 1,
                })
              }
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(v) =>
                setFilters({ ...filters, limit: Number(v ?? 20), page: 1 })
              }
            >
              <SelectTrigger>
                <SelectValue />
          ...
          <div className="md:col-span-3 lg:col-span-6 flex gap-2">
            ...
            <span className="text-xs text-muted-foreground self-center">
              Total {tx.data?.total ?? "—"} • Page{" "}
              {tx.data?.page ?? filters.page} / {totalPages}
            </span>
          </div>
        </CardContent>
      </Card>
                  ...
                  {tx.data.transactions.map((t) => {
                    return (
                      <TableRow key={t.id}>
                        ...
                        <TableCell
                          className={`font-mono text-sm ${t.direction === "debit" ? "text-destructive" : "text-emerald-600 dark:text-emerald-400"}`}
                        >
                          {t.direction === "debit" ? "−" : "+"}
          ...
          {tx.data && totalPages > 1 && (
            <div className="flex items-center justify-between p-3 border-t">
              ...
              <span className="text-xs text-muted-foreground">
                Page {filters.page} of {totalPages} • {tx.data.total} total
              </span>
              <Button
                variant="outline"
                size="sm"
