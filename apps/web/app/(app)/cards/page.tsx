...
export default function CardsPage() {
  ...
  }>({ issuer: "", last4: "", limitRupees: "", status: "active" });

  const submitCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    const paise = amountToPaiseOrNull(form.limitRupees);
    if (!paise) {
      toast.add({ title: "Invalid limit", type: "error" });
      return;
    }
    if (!/^\d{4}$/.test(form.last4)) {
      toast.add({ title: "last4 must be 4 digits", type: "error" });
      return;
    }
    try {
  ...
  const submitEdit = async (e: React.FormEvent) => {
    ...
    if (editForm.last4) {
      if (!/^\d{4}$/.test(editForm.last4)) {
        toast.add({ title: "last4 4 digits", type: "error" });
        return;
      }
    ...
    <div className="space-y-4">
          ...
          <DialogContent>
            ...
            <form onSubmit={submitCreate} className="space-y-4">
              ...
              <AmountInput
                ...
                onChange={(v) => setForm({ ...form, limitRupees: v })}
                label="Total limit (₹) *"
      ...
      <UICard>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">All cards</CardTitle>
        </CardHeader>
            ...
            <Table>
              ...
              <TableBody>
                {data.map((c) => {
                  ...
                  const pct = c.totalLimitPaise
                    ? Math.min(
                      100,
                      Math.round((c.usedPaise / c.totalLimitPaise) * 100),
                    )
                    : 0;
                  return (
                    <TableRow key={c.id}>
        ...
        <DialogContent>
          ...
          {editingId && (
            <form onSubmit={submitEdit} className="space-y-3">
              ...
              <AmountInput
                value={editForm.limitRupees}
                onChange={(v) => setEditForm({ ...editForm, limitRupees: v })}
                label="New total limit (₹) — leave blank to keep"
                placeholder=""
              />
              <div className="space-y-2">
                <Label>Status</Label>
