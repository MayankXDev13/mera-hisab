...
export function TransactionForm({
  ...
  useEffect(() => {
    if (sourceType === "account" && accounts.data?.length) setSourceId(accounts.data[0].id);
    else if (sourceType === "credit_card" && cards.data?.length) setSourceId(cards.data[0].id);
  }, [sourceType, accounts.data, cards.data]);

  const sources = sourceType === "account" ? accounts.data ?? [] : cards.data ?? [];

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const paise = amountToPaiseOrNull(amountRupees);
    if (!paise) {
      toast.add({ title: "Invalid amount", description: "Check rupee input (max 2 decimals, >0)", type: "error" });
      return;
    }
    if (!customerId || !sourceId) {
      toast.add({ title: "Missing fields", description: "Select customer and source", type: "error" });
      return;
    }
    ...
    if (sourceType === "account") {
      ...
      if (direction === "debit" && acc && acc.currentBalancePaise < paise) {
        toast.add({ title: "Insufficient balance", description: `Account ${acc.name} has ${formatRupees(acc.currentBalancePaise)}`, type: "error" });
        return;
      }
    } else {
      const card = cards.data?.find((c) => c.id === sourceId);
      if (direction === "debit" && card) {
        const avail = card.availablePaise ?? card.totalLimitPaise - card.usedPaise;
        if (avail < paise) {
          toast.add({ title: "Card limit exceeded", description: `Available ${formatRupees(avail)}`, type: "error" });
          return;
        }
      }
    }
    ...
    try {
      ...
      toast.add({ title: "Failed to create", description: errorMsg(err), type: "error" });
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <Select value={sourceType} onValueChange={(v) => setSourceType(v as "account" | "credit_card")}>
        ...
        <div className="space-y-2">
          ...
          <Select value={sourceId} onValueChange={(v) => setSourceId(v ?? "")}>
            <SelectTrigger><SelectValue placeholder={sources.length ? "Select source" : "No sources"} /></SelectTrigger>
            <SelectContent>
              {sourceType === "account"
                ? (accounts.data ?? []).map((a) => (
                    <SelectItem key={a.id} value={a.id}>
                      {a.name} — {formatRupees(a.currentBalancePaise)} {a.status !== "active" ? "(deactivated)" : ""}
                    </SelectItem>
                  ))
                : (cards.data ?? []).map((c) => (
                    <SelectItem key={c.id} value={c.id}>
                      {c.issuer} ••{c.last4} — avail {formatRupees(c.availablePaise ?? c.totalLimitPaise - c.usedPaise)}
                    </SelectItem>
                  ))}
            </SelectContent>
          </Select>
      ...
      <div className="space-y-2">
        <Label htmlFor="occurredAt">Occurred at (optional, defaults to now)</Label>
        <Input id="occurredAt" type="datetime-local" value={occurredAt} onChange={(e) => setOccurredAt(e.target.value)} />
      </div>
